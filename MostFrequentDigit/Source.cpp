...
//		4. 2 of digit 3
//		5. 10 of digit 4
// 20-based map is used for storing counter of up to 19 for every digit,
...
int getMostFrequentDigit(ulonglong input)
{
	...
	ulonglong twentyBasedDigitsMap = 0;

	while (input != 0) {
		int digit = input % 10;
		input = input / 10;

		twentyBasedDigitsMap += (ulonglong)pow(mapBase, digit);
	}

	int mostFrequentDigit = 0;
	int maxFrequency = 0;
	for (int digit = 0; twentyBasedDigitsMap != 0; digit++, twentyBasedDigitsMap /= mapBase) {
		int frequency = twentyBasedDigitsMap % mapBase;
